...
Running this module is optional. It adds samples to the manually annotated Alternate Abbreviations
dataset using a partially random selection process that preserves the existing proportion of
captured versus non-captured alias symbols. For more information on Alternate Abbreviations and
workflow please see [README](alternate_abbreviation_README).
"""
...
# Select sample of new gene-alias pairs to add to the sample set for manual annotation
...
gene_map = {hgnc_id: aalfx.get_gene_name(hgnc_id) for hgnc_id in tqdm(missing_name_ids)}
...
# TODO: After manually annotating the new rows in alt_abbrev_annotation_to_annotate_df.xlsx, it needs to be renamed and saved 
# to alt_abbrev_annotation_**manually_annotated**_df.xlsx
